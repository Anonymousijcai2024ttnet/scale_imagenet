...
class Block_resnet_multihead(nn.Module):
    '''Depthwise conv + Pointwise conv'''
    def __init__(self, in_planes, out_planes, stride=1, last = False):
        super(Block_resnet_multihead, self).__init__()
        ...
        self.convf = Block_resnet(4*in_planes, out_planes, k=2, stride=1, padding=1, last=True, groupsici= int(4*in_planes/8))#int(4*in_planes/4))
        self.stride = stride
        self.last = last
    ...
    def forward(self, x):
        ...
        #print(outf.shape)
        n, c, w, h = outf.shape
        outf = outf.view(n, 4, int(c/4), w, h)
...
class mobilenet_v2_TT(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def make_small_network(
        ...
        for index_x, x in enumerate(cfg):
            ...
            if out_planes == last_out_planes:
                last = True
            self.layers.append(Block_resnet_multihead(in_planes, out_planes, stride=stride, last = last))
            in_planes = out_planes
