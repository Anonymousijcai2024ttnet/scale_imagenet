...
class Block_resnet_multiheadsmall_n_samll32(nn.Module):
    '''Depthwise conv + Pointwise conv'''

    def __init__(self, in_planes, out_planes, stride=1, last=False):
        super(Block_resnet_multiheadsmall_n_samll32, self).__init__()

        self.conv1 = Block_resnet(in_planes, in_planes, k=4, stride=stride, padding=1,
                                  groupsici=int(in_planes / 1))  # int(in_planes/1))
        # self.conv2 = Block_resnet(in_planes, in_planes, k = 3, stride = stride, padding=1, groupsici = int(in_planes/2))#int(in_planes/2))
        self.conv3 = Block_resnet(in_planes, in_planes, k=2, stride=stride, padding=0,
                                  groupsici=int(in_planes / 4))  # int(in_planes/4))
        ...
        self.convf = Block_resnet(2 * in_planes, out_planes, k=2, stride=1, padding=1, last=True,
                                  groupsici=int(2 * in_planes / 4))  # int(4*in_planes/4))
        self.stride = stride
        self.last = last
...
class mobilenet_v2_TT_32_32(SeqBinModelHelper, nn.Module, ModelHelper):
    CLASS2NAME = tuple(map(str, range(10)))
    ...
    def make_small_network(
        ...
        for index_x, x in enumerate(cfg):
            ...
            if out_planes == last_out_planes:
                last = True
            self.layers.append(Block_resnet_multiheadsmall_n_samll32(in_planes, out_planes, stride=stride, last=last))
            in_planes = out_planes
