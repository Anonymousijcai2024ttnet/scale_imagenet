...
class Block_resnet_multihead_general_BN_vf_small_v2(nn.Module):
    ...
    def forward(self, x):
        ...
        pad2 = nn.ZeroPad2d((0, 0, 0, 1))
        pad3 = nn.ZeroPad2d((0, 1, 0, 0))
        #print(out1.shape, out2.shape, out3.shape, out4.shape, x.shape)
        if (x.shape[-1] == 16): #or (x.shape[-1] == 9) or (x.shape[-1] == 10):
            out1 = pad2(out1)
            out2 = pad3(out2)
            out3 = pad(out3)
            out4 = pad(out4)
...
class TT_FHE_small(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def make_small_network(
        ...
        t = self.args.tfilter
        p = n * t
        self.layers = [nn.Conv2d(3, p, kernel_size=3, stride=2, padding=1, groups=1, bias=False)]
        ...
        self.layers.append(nn.Linear(fcsize, 100))
