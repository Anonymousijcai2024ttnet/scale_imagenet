...
class Polynome_ACT(nn.Module):
    ...
    def forward(self, input):
        out = 0.47 + 0.50 * input  + 0.09 * input[:100 ]**2 #- 1.7e-10 * input**3 #self.alpha * self.h_function((input / self.gamma) + self.beta) - self.alpha * self.h2_function(self.beta)
        return out
...
class Block_resnet_multihead_general_BN_vf_small_v3(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, T=0.0, last=False):
        ...
        #else:
        #    self.Block_convf = Block_TT(self.cpt * in_planes, 4 * 64, k=1, stride=1,
        #                                   padding=0,
        #                                   groupsici=int(self.cpt * in_planes / groupvf), T=T, last=last)  # int(4*in_planes/4))
    ...
    def forward(self, x):
        ...
        if (x.shape[-1] == 16) and out1.shape[-1]==17: #or (x.shape[-1] == 9) or (x.shape[-1] == 10):
            ...
            out4 = pad(out4)
            #out1 = pad(out1)
        #elif (x.shape[-1] == 8):  # and out1.shape[-1]==16: #or (x.shape[-1] == 9) or (x.shape[-1] == 10):
                # pass
            #out1 = pad21(out1)
            #out2 = pad31(out2)
            #out3 = pad(out3)
...
class TT_FHE_XSMALL_vAlexnet(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def __init__(self, args):
        ...
        VGG_Model16 = models.vgg16(pretrained=True)
        #AlexNet_Model = torch.hub.load('pytorch/vision:v0.6.0', 'alexnet', pretrained=True)
        #self.model_alex = AlexNet_Model.features[0].eval()
        model_ft = models.densenet121(pretrained=True)
        # self.VGG_Model16_0 = model_ft.features[0].eval()
        # self.VGG_Model16_1 = model_ft.features[1].eval()
        # self.VGG_Model16_2 = model_ft.features[2].eval()
        # self.VGG_Model16_3 = model_ft.features[3].eval()
        # self.VGG_Model16_4 = model_ft.features[0].eval()
        self.VGG_Model16_0 = VGG_Model16.features[0].eval()
        self.VGG_Model16_1 = VGG_Model16.features[1].eval()

        #self.VGG_Model16_2 = VGG_Model16.features[2].eval()
        #self.VGG_Model16_3 = VGG_Model16.features[3].eval()
        #self.VGG_Model16_4 = VGG_Model16.features[4].eval()
    ...
    def make_small_network(
        ...
        t = self.args.tfilter
        p = n * t
        self.layers = [self.VGG_Model16_0, self.VGG_Model16_1]#, self.VGG_Model16_2, self.VGG_Model16_3]#, self.VGG_Model16_4] #[self.model_alex] #nn.Conv2d(3, p, kernel_size=5, stride=2, padding=2, groups=1, bias=False)]
        cfg = [p]
        self.T = 0.0
        self.T_block = 0.0
        self.layers.append(nn.BatchNorm2d(64))
        self.layers.append(nn.MaxPool2d(3)) # maxpool3: 74.1 // 5: 71.86% // 4: 72.55 // maxpool 3, only concat (out1, out3, x) - 73.04%
        self.layers.append(Binarize01Act(T=self.T))
        in_planes = 64 #p
        last = False
        last_out_planes = 64 #cfg[-1] if isinstance(cfg[-1], int) else cfg[-1][0]
