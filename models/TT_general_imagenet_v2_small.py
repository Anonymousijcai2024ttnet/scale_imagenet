...
class Block_resnet_multihead_general_BN_vf_imgnet_v2small(nn.Module):
    '''Depthwise conv + Pointwise conv'''

    def __init__(self, in_planes, out_planes, groups, stride=1, Abit_inter=1, last=False):
        super(Block_resnet_multihead_general_BN_vf_imgnet_v2small, self).__init__()
        self.cpt = 0
        self.pad0 = nn.ZeroPad2d((1,0,1,0))
        self.groups = [1,1,16,1]
        ...
        for index_g, g in enumerate(self.groups):
            ...
            if g is not None:
                if index_g == 0:
                    #pass
                    self.Block_conv1 = Block_TT(in_planes, in_planes, k = (4), stride=stride, padding=2,
                                   groupsici=int(in_planes / g))  # int(in_planes/1))
                    #self.Block_conv1 = nn.AvgPool2d(2)
                    self.cpt += 1

                elif index_g == 1:
                    self.Block_conv2 = Block_TT(in_planes, in_planes, k=(4), stride=stride,
                                                       padding=2,
...
class TT_vf_19lv3_imgnet_small(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def make_small_network(
        ...
        for index_x, x in enumerate(cfg):
            ...
            if out_planes == last_out_planes:
                last = True
            self.layers.append(Block_resnet_multihead_general_BN_vf_imgnet_v2small(in_planes, out_planes, self.args.groups,
