...
class Block_resnet_multihead_general_BN_vf_imgnet_v2xsmall(nn.Module):
    '''Depthwise conv + Pointwise conv'''

    def __init__(self, in_planes, out_planes, groups, stride=1, Abit_inter=1, last=False):
        super(Block_resnet_multihead_general_BN_vf_imgnet_v2xsmall, self).__init__()
        self.cpt = 0
    ...
    def forward(self, x):
        ...
        return outf #self.Block_convf(outf)

import torchvision.models as models
class TT_vf_19lv3_imgnet_xsmall_VGG(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def make_small_network(
        ...
        self.layers = [nn.AvgPool2d(4), self.VGG_Model16_0, self.VGG_Model16_1, self.VGG_Model16_2, self.VGG_Model16_3]#, self.VGG_Model16_4] #[self.model_alex] #nn.Conv2d(3, p, kernel_size=5, stride=2, padding=2, groups=1, bias=False)]
        #param.requires_grad = True
        # SUR 150 epochs
        #if self.args.layers == 0:
        cfg = [(p) ]#, (2 * p, 2)] #- Acc1: 44.902% Acc5: 68.556%
        ...
        for index_x, x in enumerate(cfg):
            ...
            if out_planes == last_out_planes:
                last = True
            self.layers.append(Block_resnet_multihead_general_BN_vf_imgnet_v2xsmall(in_planes, out_planes, self.args.groups,
