...
class Block_resnet_multiheadsmall(nn.Module):
    '''Depthwise conv + Pointwise conv'''
    def __init__(self, in_planes, out_planes, stride=1, last = False):
        super(Block_resnet_multiheadsmall, self).__init__()
        
        self.conv1 = Block_resnet(in_planes, in_planes, k = 4, stride = stride, padding=1, groupsici = int(in_planes/1))#int(in_planes/1))
        #self.conv2 = Block_resnet(in_planes, in_planes, k = 3, stride = stride, padding=1, groupsici = int(in_planes/3))#int(in_planes/2))
        #self.conv3 = Block_resnet(in_planes, in_planes, k = 2, stride = stride, padding=0, groupsici = int(in_planes/6))#int(in_planes/4))
        self.conv4 = nn.MaxPool2d(2)
        self.act = activation_quantize_fn2()#Binarize01Act()
        #print(4*in_planes, out_planes, int(4*in_planes/6))
        self.convf = Block_resnet(2*in_planes, out_planes, k=2, stride=1, padding=1, last=True, groupsici= int(2*in_planes/4))#int(4*in_planes/4))
        self.stride = stride
        self.last = last
    ...
    def forward(self, x):
        ...
        outf = torch.cat(( out1, out4), axis = 1)
        #print(outf.shape)
        n, c, w, h = outf.shape
...
class mobilenet_v2_TT_small(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def make_small_network(
        ...
        t = 8
        p = n * t
        cfg = [p, (2 * p, 2), (4 * p, 2), (8 * p, 2)]#, 4 * p, (8 * p, 2)]
        #, 8 * p, 8 * p, 8 * p, 8 * p, 8 * p, (16 * p, 2), 16 * p]
        ...
        for index_x, x in enumerate(cfg):
            ...
            if out_planes == last_out_planes:
                last = True
            self.layers.append(Block_resnet_multiheadsmall(in_planes, out_planes, stride=stride, last = last))
            in_planes = out_planes
