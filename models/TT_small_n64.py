...
class Block_resnet_multiheadsmall_n64(nn.Module):
    '''Depthwise conv + Pointwise conv'''
    def __init__(self, in_planes, out_planes, stride=1, last = False):
        super(Block_resnet_multiheadsmall_n64, self).__init__()
...
class mobilenet_v2_TT_small_n64(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def make_small_network(
        ...
        for index_x, x in enumerate(cfg):
            ...
            if out_planes == last_out_planes:
                last = True
            self.layers.append(Block_resnet_multiheadsmall_n64(in_planes, out_planes, stride=stride, last = last))
            in_planes = out_planes
